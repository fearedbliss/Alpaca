...
FCT_ASM(caller_handleClientUpdate_9)
...
void Install_UpdateClient()
{
	...
	DWORD ExecuteOurPacketsOnlyOffset = D2Client::GetAddress(0x14236);

	// execute if it's our packet else continue
	Memory::SetCursor(ExecuteOurPacketsOnlyOffset);
	Memory::ChangeCallA(0xD6, (DWORD)caller_handleClientUpdate_9);
