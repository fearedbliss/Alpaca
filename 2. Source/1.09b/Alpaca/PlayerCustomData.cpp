...
Unit* __fastcall updateItem_9(GameStruct* ptGame, DWORD type, DWORD itemNum, Unit* ptChar)
{
...
void Install_PlayerCustomData()
{
	...
	DWORD InitializeCustomDataOffset = D2Common::GetAddress(0x70462);
	DWORD UpdateItemOffset1 = D2Game::GetAddress(0x10933);
	DWORD UpdateItemOffset2 = D2Game::GetAddress(0x1097B);
	DWORD UpdateClientOnLoadingOffset = D2Game::GetAddress(0x23EB);
	DWORD FreeCustomDataOffset = D2Common::GetAddress(0x7055C);
	DWORD FreeItemInStashServerSideOffset = D2Game::GetAddress(0x7D12B);
	DWORD FreeItemInStashClientSideOffset = D2Client::GetAddress(0x8EF8F);
	DWORD AdditionalOffset1 = D2Game::GetAddress(0x7D176);
	DWORD AdditionalOffset2 = D2Client::GetAddress(0x8F0CA);
	DWORD AdditionalOffset3 = D2Client::GetAddress(0x8F13C);
	...
	Memory::ChangeCallC((DWORD)D2Game::D2GameGetObject, (DWORD)caller_updateItem_9);
	...
	Memory::ChangeCallB((DWORD)D2Common::D2UnitGetNextItem, (DWORD)callerServer_getNextItemToFree_9);

	// Free item in Stash (Client-side)
	Memory::SetCursor(FreeItemInStashClientSideOffset);
	Memory::ChangeCallB((DWORD)D2Common::D2UnitGetNextItem, (DWORD)callerClient_getNextItemToFree_9);

	Memory::SetCursor(AdditionalOffset1);
	Memory::ChangeByte(0x74, 0x90);
	Memory::ChangeByte(0x35, 0x90);

	Memory::SetCursor(AdditionalOffset2);
	Memory::ChangeByte(0x0F, 0x90);
	Memory::ChangeByte(0x84, 0x90);
	Memory::ChangeDword(0xBF, 0x90909090);

	Memory::SetCursor(AdditionalOffset3);
	Memory::ChangeByte(0x74, 0x90);
	Memory::ChangeByte(0x6F, 0x90);
